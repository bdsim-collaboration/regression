...
def test(test_length, testlength_primaries, testdata_store):
    ...
    assert pytest.approx(sampler_Sigma_x, rel=1e-3)  == 0.015804539022049573
    assert pytest.approx(sampler_Sigma_xp, rel=1e-3) == 0.010316564235008652
    assert pytest.approx(sampler_mean_x, rel=1e-3)   == 8.499772373153859e-05
    assert pytest.approx(sampler_Sigma_y, rel=1e-3)  == 0.0021579719729259575
    assert pytest.approx(sampler_Sigma_yp, rel=1e-3) == 0.0023403134007770198
    assert pytest.approx(sampler_mean_y, rel=1e-3)   == 1.8904866732009396e-05
