...
def test(test_length, testlength_primaries, testdata_store):
    ...
    assert pytest.approx(sampler_Sigma_x, rel=1e-3)  == 0.01438563542490261
    assert pytest.approx(sampler_Sigma_xp, rel=1e-3) == 0.007424089790444462
    assert pytest.approx(sampler_mean_x, rel=1e-3)   == 0.577986688876152
    assert pytest.approx(sampler_Sigma_y, rel=1e-3)  == 0.005241687258850288
    assert pytest.approx(sampler_Sigma_yp, rel=1e-3) == 0.002609618738995901
    assert pytest.approx(sampler_mean_y, rel=1e-3)   == 0.25584089472591875
