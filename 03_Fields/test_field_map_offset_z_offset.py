...
def test(test_length, testlength_primaries, testdata_store):
    ...
    assert pytest.approx(sampler_Sigma_x, rel=1e-3)  == 0.013459971494015222
    assert pytest.approx(sampler_Sigma_xp, rel=1e-3) == 0.007452970502992219
    assert pytest.approx(sampler_mean_x, rel=1e-3)   == 0.5479260095000267
    assert pytest.approx(sampler_Sigma_y, rel=1e-3)  == 0.004177156229900975
    assert pytest.approx(sampler_Sigma_yp, rel=1e-3) == 0.002217752745742983
    assert pytest.approx(sampler_mean_y, rel=1e-3)   == 0.2142822514116764
