...
def test(test_length, testlength_primaries, testdata_store):
    ...
    assert pytest.approx(sampler_Sigma_x, rel=1e-3)  == 0.0017761945113131978
    assert pytest.approx(sampler_Sigma_xp, rel=1e-3) == 0.00035136689967708905
    assert pytest.approx(sampler_mean_x, rel=1e-3)   == 1.3215959309269466e-05
    assert pytest.approx(sampler_Sigma_y, rel=1e-3)  == 0.0017857934386989697
    assert pytest.approx(sampler_Sigma_yp, rel=1e-3) == 0.0003543900315124393
    assert pytest.approx(sampler_mean_y, rel=1e-3)   == -2.1371774069270798e-05
