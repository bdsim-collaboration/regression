...
def test(test_length, testlength_primaries, testdata_store):
    ...
    assert pytest.approx(sampler_Sigma_x, rel=1e-3)  == 0.005376337095341065
    assert pytest.approx(sampler_Sigma_xp, rel=1e-3) == 0.0020095770501316365
    assert pytest.approx(sampler_mean_x, rel=1e-3)   == 0.009675747349709855
    assert pytest.approx(sampler_Sigma_y, rel=1e-3)  == 0.013080372518385144
    assert pytest.approx(sampler_Sigma_yp, rel=1e-3) == 0.007200602146071492
    assert pytest.approx(sampler_mean_y, rel=1e-3)   == -0.4976539144992828
