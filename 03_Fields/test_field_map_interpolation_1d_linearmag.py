...
def test(test_length, testlength_primaries, testdata_store):
    ...
    assert pytest.approx(sampler_Sigma_x, rel=1e-3)  == 0.005360657975868763
    assert pytest.approx(sampler_Sigma_xp, rel=1e-3) == 0.0020020719479500963
    assert pytest.approx(sampler_mean_x, rel=1e-3)   == 0.009683479112656596
    assert pytest.approx(sampler_Sigma_y, rel=1e-3)  == 0.01307943167235869
    assert pytest.approx(sampler_Sigma_yp, rel=1e-3) == 0.0072005010788836685
    assert pytest.approx(sampler_mean_y, rel=1e-3)   == -0.49720062289237976
