...
def test(test_length, testlength_primaries, testdata_store):
    ...
    assert pytest.approx(sampler_Sigma_x, rel=1e-3)  == 0.014324369577942097
    assert pytest.approx(sampler_Sigma_xp, rel=1e-3) == 0.007515015279365734
    assert pytest.approx(sampler_mean_x, rel=1e-3)   == 0.5660426369667053
    assert pytest.approx(sampler_Sigma_y, rel=1e-3)  == 0.0044860079849747825
    assert pytest.approx(sampler_Sigma_yp, rel=1e-3) == 0.002270031791075037
    assert pytest.approx(sampler_mean_y, rel=1e-3)   == 0.22305434429049492
