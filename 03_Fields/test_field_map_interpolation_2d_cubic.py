...
def test(test_length, testlength_primaries, testdata_store):
    ...
    assert pytest.approx(sampler_Sigma_x, rel=1e-3)  == 0.018419807674184555
    assert pytest.approx(sampler_Sigma_xp, rel=1e-3) == 0.012275748730702854
    assert pytest.approx(sampler_mean_x, rel=1e-3)   == 0.00010576244183662311
    assert pytest.approx(sampler_Sigma_y, rel=1e-3)  == 0.0028903558879167703
    assert pytest.approx(sampler_Sigma_yp, rel=1e-3) == 0.0032919311481443892
    assert pytest.approx(sampler_mean_y, rel=1e-3)   == 1.0439962574434957e-05
